...
void InitializeConsole() {
  ...
  HANDLE hConOut = CreateFile(L"CONOUT$", GENERIC_READ | GENERIC_WRITE,
...
void OpenLicenses() {
  ...
  ::ShellExecuteW(NULL, L"open", D3d12infoGUILicense.c_str(), NULL, NULL,
                  SW_SHOW);
#if !USE_D3D12INFO_FROM_WORKING_DIRECTORY
  auto D3d12infoLicense = rootPath / L"D3d12info_LICENSE.txt";
  ::ShellExecuteW(NULL, L"open", D3d12infoLicense.c_str(), NULL, NULL,
                  SW_SHOW);
#endif
}

int ProcessCommandLine(LPWSTR lpCmdLine) {
  if (std::wcscmp(lpCmdLine, L"--licenses") == 0 ||
      std::wcscmp(lpCmdLine, L"-l") == 0) {
    OpenLicenses();
    return 0;
  }

  if (std::wcscmp(lpCmdLine, L"--help") == 0 ||
      std::wcscmp(lpCmdLine, L"-h") == 0) {
    PrintHelp();
    return 0;
  }

...
{
    Subprocess::ProcessOutput Subprocess::GetCommandOutput(std::string_view commandLine)
    {
    ...
    void Subprocess::InitializePipe(HANDLE& stdoutRead, HANDLE& stdoutWrite)
    {
        ...
        if(!::SetHandleInformation(stdoutRead, HANDLE_FLAG_INHERIT, 0))
            throw std::runtime_error("Subprocess::InitializePipe::SetHandleInformation");
    }
    void Subprocess::LaunchProcess(
        HANDLE stdinRead, HANDLE stdoutWrite, HANDLE& processHandle, std::string_view commandLine)
    {
        ...
        BOOL bSuccess = ::CreateProcessA(
            NULL, commandLineCopy.data(), NULL, NULL, TRUE, CREATE_NO_WINDOW, NULL, NULL, &startupInfo, &procInfo);
        if(!bSuccess)
    ...
    std::vector<char> Subprocess::ReadOutput(HANDLE stdoutRead)
    {
        ...
        for(;;)
        {
            BOOL bSuccess = ::ReadFile(stdoutRead, tempBuffer, ARRAYSIZE(tempBuffer), &bytesRead, NULL);
